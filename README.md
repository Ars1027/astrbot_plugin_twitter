...
_✨ 支持 Nitter 与 FxTwitter API 双数据源的 Twitter 推文转发插件，提供多会话独立订阅、定时推送、链接识别、合并转发与推文翻译。 ✨_
...
FxTwitter 时间线使用有限 cursor 分页并在本地按推文 ID 去重、筛选和排序；首次关注只记录最新 ID，不回放历史。API 当前返回的媒体 URL 可能仍属于 `pbs.twimg.com` / `video.twimg.com`，网络受限环境可配置 `twitter_proxy` 并开启 `twitter_pre_download_media`。
