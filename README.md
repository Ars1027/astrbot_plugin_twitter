...
页面支持跟随 Dashboard 切换明暗主题，并适配手机布局；可搜索会话，以及当前会话中的推主名称或用户名。

点击订阅中的推主名称可查看**当前会话最近 5 条成功自动推送**，按推送时间倒序显示摘要和原帖链接。
记录仅从本次更新后的推送开始积累，不用旧游标或已处理 ID 推断历史；失败、过滤跳过和仅入队的推文不计入。
每条摘要最多 500 个字符，不保存图片、视频文件，不为查看记录额外请求 Twitter。
记录保存在对应订阅关系的可选 `recent_deliveries` 字段中，插件重载后保留，取关或清空订阅时一并删除；旧订阅默认显示空记录，无需迁移。
这一记录用于展示，写入失败不会改变发送结果或触发重新推送；发送成功沿用现有框架调用结果，不是客户端已读回执。

在已连接的群聊中点击 **批量添加**，粘贴用户名或 `@用户名`，用换行、空格、中英文逗号或分号分隔。
预览会区分待添加、已订阅、重复及无效项；每批最多 100 个不同的有效账号，超过时请分批处理。
R18 和仅媒体选项统一应用于本批新订阅，已订阅账号会跳过，原有设置保持不变。

批量添加按顺序执行，展示每项结果。**停止后续添加** 会等待当前请求结束，已经成功的订阅不会撤销。
请求报错不一定代表服务端未保存，使用 **核对并重试未完成项** 时会先刷新订阅，再跳过已经存在的账号。
批量队列只在当前页面中运行，关闭页面后不再发送后续请求，已发出的请求仍可能完成；不支持后台续跑。

旧版 AstrBot 缺少 Plugin Pages API 时不会显示该页面，但插件仍可正常加载，聊天指令和自动推送功能不受影响。WebUI 中的轮询间隔仍是全局配置，对所有会话生效。

开发时可运行 `python tests/webui_preview.py`，访问 `http://127.0.0.1:8765` 查看模拟页面。
该预览仅监听本机，数据保存在当前页面内存中，不连接真实群或写入订阅 KV，刷新浏览器即重置。
预览中的 `retry_demo` 首次添加会模拟失败，便于检查重试；左下角可切换明暗主题。
前端解析与队列测试运行 `node --test tests/test_webui_batch.mjs`（仅开发测试需要 Node.js）。
...
| `/推特列表` | `/twitter_list` | 以分段合并转发查看当前会话的完整订阅列表 |
...
发送一次 `/推特列表` 或 `/twitter_list`，即可收到包含全部订阅的一条合并转发消息，
无需输入页码。聊天记录内按多个节点分段，每段显示订阅总数、当前段号和总段数。
每段固定最多 **50 条订阅**，且标题和正文合计不超过 **1000 字符**；
达到任一限制就分段，名称较长时每段可能少于 50 条。这两个限制不提供配置项。
展示名中的连续空白统一为空格，超过 50 字符时以省略号截短；账号 ID、状态和存储数据不变。
无订阅时仍回复普通文字提示。

分段用于避免单个转发节点过长。1000 字符是插件的输出预算，不是 QQ 协议上限；
该指令直接构造合并转发，不依赖 AstrBot 的长文本转发阈值或推文发送模式配置。
