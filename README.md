...
| `/推特列表 [页码]` | `/twitter_list [页码]` | 分页查看当前会话的订阅列表，默认第 1 页 |
...
发送 `/推特列表` 查看第一页，发送 `/推特列表 2` 查看第二页，英文别名同样支持页码。
回复显示订阅总数、当前页、总页数和下一页命令。
每页固定最多 **50 条订阅**，且标题、正文和翻页提示合计不超过 **1000 字符**；
达到任一限制就分页，名称较长时每页可能少于 50 条。这两个限制不提供配置项。
展示名中的连续空白统一为空格，超过 50 字符时以省略号截短；账号 ID、状态和存储数据不变。
翻页按查询时的订阅列表计算，期间增删订阅可能改变页面位置。

分页用于避免一次输出过长。1000 字符是插件的输出预算，不是 QQ 协议上限；
AstrBot 的回复前缀、转发阈值和其他插件仍可能影响最终显示形式。
