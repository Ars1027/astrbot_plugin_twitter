...
分页失败保留已保存页面；同一非空 cursor 连续 3 轮失败或出现循环时，下一轮从首页重新确认完整性，保留已抓条目并去重。更换数据源地址也会重新扫描，不复用其他地址的 cursor。重扫允许并入新出现的推文，不再用首页最大原帖 ID 截断转帖。条目按时间线位置从旧到新交付；重扫中未重新出现的已保存条目保留相对顺序并优先交付。已完整的积压先消化完，再使用当前来源抓取新批次。

为限制资源占用，单推主积压上限为 5,000 条元数据、8 MiB 序列化状态、单次遍历 1,000 个分页 cursor（内部常量）。达到上限时保留最后完整页面并暂停该推主、记录错误，其他推主继续；需要提高 `TimelineBacklogService` 中相应上限并重载才能继续，不会自动丢弃积压。未知版本或损坏的状态同样保留并暂停，需检查日志和备份后修复。

旧订阅会惰性创建积压，无须手动迁移。积压内部格式为 version=2；有效的 v1 积压保留未发送条目和已确认处理记录，重新扫描以修复旧上限可能造成的缺口，原容量阻塞仍保留。已被旧实现永久清除且上游不再提供的记录无法恢复；历史记录不足以确认是否发送时，迁移优先保留，可能有限重复。回退到不支持 `timeline_backlog` 的旧插件版本后，旧版无法保证利用未完成积压恢复；有未完成积压时请避免降级。以上保障以数据源仍提供有效分页结果且积压未超过容量为前提，不能恢复上游已删除或隐藏的推文，也不保证发送与本地 KV 写入之间恰好一次投递。
